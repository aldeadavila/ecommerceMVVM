...
@Composable
fun GetProducts(navHostController: NavHostController, paddingValues: PaddingValues, vm: AdminProductListViewModel = hiltViewModel()) {
    ...
    when(val response = vm.productResponse) {
        ...
        is Resource.Succes -> {
           ...
           AdminProductListContent(navHostController, paddingValues = paddingValues, products = response.data)
